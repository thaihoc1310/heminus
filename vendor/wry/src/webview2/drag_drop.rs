...
impl IDropTarget_Impl for DragDropTarget_Impl {
  fn DragEnter(
    ...
    let screen_point = POINT { x: pt.x, y: pt.y };
    ...
    if enter_is_valid {
      (self.listener)(DragDropEvent::Enter {
        paths,
        position: (client_point.x as _, client_point.y as _),
      });
    }
  ...
  fn DragOver(
    ...
    unsafe {
      ...
      *pdwEffect = effect;
      if let Some(helper) = &self.drag_image_helper {
        let _ = helper.DragOver(&screen_point, effect);
      }
    }
    Ok(())
  }
  ...
  fn Drop(
    ...
    unsafe {
      ...
      *pdwEffect = effect;
      if let Some(helper) = &self.drag_image_helper {
        let _ = helper.Drop(pDataObj.as_ref(), &screen_point, effect);
      }
    }
    if unsafe { *self.enter_is_valid.get() } {
      let mut client_point = screen_point;
