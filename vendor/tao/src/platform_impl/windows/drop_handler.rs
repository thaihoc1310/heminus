...
use windows::{
  core::implement,
  Win32::{
    Foundation::{self as win32f, HWND, POINT, POINTL},
    System::{
      Com::{
    ...
    UI::Shell::{
      DragFinish, DragQueryFileW, IDropTargetHelper, CLSID_DragDropHelper, HDROP,
    },
  },
};
    ...
    unsafe {
      ...
      *pdwEffect = cursor_effect;
      if let Some(helper) = &self.drag_image_helper {
        let point = POINT { x: _pt.x, y: _pt.y };
        let _ = helper.DragEnter(self.window, pDataObj.as_ref(), &point, cursor_effect);
      }
    }
    Ok(())
  }
  ...
  fn DragOver(
    ...
    unsafe {
      *pdwEffect = *self.cursor_effect.get();
      if let Some(helper) = &self.drag_image_helper {
        let point = POINT { x: _pt.x, y: _pt.y };
        let _ = helper.DragOver(&point, *self.cursor_effect.get());
      }
    }
    Ok(())
  }
  ...
  fn Drop(
    ...
    unsafe {
      if let Some(helper) = &self.drag_image_helper {
        ...
        let effect = *self.cursor_effect.get();
        let _ = helper.Drop(pDataObj.as_ref(), &point, effect);
        *pdwEffect = effect;
      }
      let hdrop = FileDropHandler::iterate_filenames(&pDataObj, |filename| {
        (self.send_event)(Event::WindowEvent {
